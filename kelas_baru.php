...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		...
		if(($submit=='hapus') AND isset($_REQUEST['nis'])){
			$nis = $_REQUEST['nis'];
			$qsiswa = mysql_query("DELETE FROM kelas WHERE kelas='$kelas' AND th_pelajaran='$tapel' AND nis='$nis'");
		}
		...
		$qkelas = mysql_query("SELECT nis FROM kelas WHERE kelas='$kelas' AND th_pelajaran='$tapel'");
...
//-->
<h2 style="color: red;">Tambah Kelas</h2><hr>
<form method="post" action="admin.php?hlm=master&sub=kelas&aksi=view" class="form-horizontal" role="form">
	<div class="form-group">
		<label style="color: black;"for="kelas" class="col-sm-2 control-label">Kelas</label>
		<div class="col-sm-2">
			<input type="text" class="form-control" id="kelas" name="kelas" placeholder="Kelas" required autofocus>
		</div>
	</div>
	<div class="form-group">
		<label style="color: black;" for="tapel" class="col-sm-2 control-label">Tahun Pelajaran</label>
		<div class="col-sm-2">
	...
	<div class="form-group">
		<label style="color: black;" for="prodi" class="col-sm-2 control-label">Program Studi</label>
		<div class="col-sm-3">
